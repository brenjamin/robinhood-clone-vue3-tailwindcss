...
export default {
    ...
    setup() {
            ...
            timeout = setTimeout(() => {
                if (searchTerm.value) {
                    ...
                    searchResults.value = searchResults.value.map((item) => {
                        item.symbol = item.symbol.replace(matchThis, '<span class="text-green dark:text-red">$&</span>')
        ...
        return { toggleDarkMode, searchTerm, updateStocks, searchResults, searchIsFocused }
    }
